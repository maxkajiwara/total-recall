...
export default function DesktopNavigation({ 
  ...
  return (
    <div className="w-full border-b border-custom bg-surface">
            ...
            {navigationItems.map((item) => (
              <button
                ...
                className={`relative px-4 py-2 rounded-lg text-sm font-medium transition-colors focus-ring ${
                  currentView === item.id
                    ? 'bg-primary-light text-primary-custom'
                    : 'text-secondary hover:text-primary-custom hover:bg-surface-hover'
                }`}
              >
                {item.label}
                {currentView === item.id && (
